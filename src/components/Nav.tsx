...
const Nav: FC = () => {
  ...
  return (
    <AnimateSharedLayout>
      <motion.nav
        className={`flex py-6 px-8 md:px-24 justify-between items-center sticky top-0 bg-bg-primary z-50 border-b border-white overflow-hidden ${
          isTabletOrMobile ? "flex-col" : "flex-row"
        }`}
        ref={navRef}
        animate={{ y: 0, transition: { duration: 0.6, type: "spring" } }}
        initial={{ y: -100 }}
        layout
      >
        <motion.div
        ...
        >
          ...
          >
            <motion.div
              className="border border-white rounded-full flex justify-center items-center overflow-hidden h-12 w-12"
              layout
            >
